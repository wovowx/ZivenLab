...
### 验证（2026-09-06 00:10 初步观察·待确证）
新对话 `6a9c3dbc`（zivencheng 长期计划 GPTs）+ v7 注入，chat2api 连续两次发消息调 `ds_quota` 都成功（0.45 CNY）。
**⚠️ 不能据此下「注入成功」结论**：新框开头柳柳在**页面**注入过插件，对话上下文带插件状态——chat2api 发的第二条消息可能在插件「有效期」内，能调可能是**对话级残留**，不是 v7 注入生效。
真正判据（二选一）：
1. **纯对照组**：新对话**第一条消息**就由 chat2api 发、页面从不挂插件 → 能调 ds_quota 才是注入生效
2. **聊一段时间后**：对话上下文忘记插件 → 再看能否调用
