...
> **解决什么问题**：chat2api 直接部署在云平台（数据中心 IP，曾用 Google Cloud Run）时，高频调用会被上游 ChatGPT 风控拦截（`cf_chl_opt` / 403 / IP 太脏）。
>
...
## 架构（2026-09-11 更新，平台已迁 Northflank）

- **服务名**：`liu--zivenlab--x7t9qxpv5vy6`，URL https://liu--zivenlab--x7t9qxpv5vy6.code.run（**Northflank**，端口 5005 HTTP/1，Public）
- **源码构建**：Northflank 自动构建（仓库 wovowx/ZivenLab 分支 dev → Build context `chatroom/xray`，Dockerfile `chatroom/xray/Dockerfile`）——不用 gcloud、不用 Cloud Shell
- **节点来源（三层容灾）**：
...
## 构建 & 部署（当前：Northflank 控制台，完整流程见 DEPLOY.md §3.5）
...
- **换 specified 节点**：改 ZivenLab dev `node-config.json` 的 `specified_nodes` 数组 → 推代码 → Northflank 控制台 Restart 服务
